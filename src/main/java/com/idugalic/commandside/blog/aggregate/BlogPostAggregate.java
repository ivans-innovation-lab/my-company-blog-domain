...
import com.idugalic.common.blog.event.BlogPostPublishErrorEvent;
...
class BlogPostAggregate {
	 ...
	 * BlogPostAggregate's Id in order to make the Aggregate reflect it's true
	 * logical state.
	 */
	...
	/**
	 * This constructor is marked as a 'CommandHandler' for the AddProductCommand.
	 * This command can be used to construct new instances of the Aggregate. If
	 * successful a new BlogPostAggregate is 'applied' to the aggregate using the
	 ...
	 * Axon framework to handle events of the specified type (BlogPostCreatedEvent).
	 * The BlogPostCreatedEvent can be raised either by the constructor during
	 * BlogPostAggregate(CreateBlogPostCommand) or by the Repository when
